...
$sql = 'SELECT parent_id FROM ' . $GLOBALS['ecs']->table('category') . " WHERE is_show =1";
...
if ($GLOBALS['db']->getOne($sql) || $parent_id == 0)
{
    ...
    foreach ($res AS $row)
    {
        if ($row['is_show'])
        {
            ...
            $cat_arr[$row['cat_id']]['url']  = 'category.php?id='.$row['cat_id'];

        }
    }
}

$smarty->assign('categoriest',$cat_arr); // 分类树
...
function get_also_bought($goods_id)
{
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
		...
		$arr[$key]['source_img']   = get_image_path($row['goods_id'], $row['source_img']);
