...
//        var_dump($_FILES);die;
        ...
        $sql = "update app_config set val='images/".$file_name."' where k='url'";
        $GLOBALS['db']->query($sql);
    }
    $version = $_POST['version_name'];
    $sql = "update app_config set val  ='".$version."' where k ='version'";
    $GLOBALS['db']->query($sql);
    $sql = "update app_config set val  ='".$_POST['day']."' where k ='day'";
    $GLOBALS['db']->query($sql);
    $sql = "update app_config set val  ='".$_POST['tel']."' where k ='tel'";
    $GLOBALS['db']->query($sql);
    $links[] = array('text' => $_LANG['go_url'], 'href' => 'mobile_setting.php?act=img_setting');
    sys_msg($_LANG['edit_ok'], 0, $links);
