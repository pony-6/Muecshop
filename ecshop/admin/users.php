...
elseif ($_REQUEST['act'] == 'edit')
{
    ...
    //以下是将昵称进行解码
    $user['user_name'] = urldecode($user['user_name']);
...
function user_list()
{
    ...
    if ($result === false)
    {
        ...
        $sql = "SELECT user_id, user_name, email,wx_status, is_validated, user_money, frozen_money, rank_points, pay_points, reg_time ".
    ...
    //因为微信昵称使用urlencode编码，所以会出现有些名字是乱码的情况
    //这边开始解码
    ...
    for ($i=0; $i<$count; $i++)
    {
        ...
        if($user_list[$i]['wx_status']=='true'){
            $user_list[$i]['user_name'] =  utf8_decode($user_list[$i]['user_name']);
        }
