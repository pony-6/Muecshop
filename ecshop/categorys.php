...
if (!$smarty->is_cached('pc_category.dwt', $cache_id))
{
    /* 如果页面没有被缓存则重新获取页面的内容 */
   ...
   // $smarty->assign('categories',       get_categories_tree($cat_id)); // 分类树
   	$smarty->assign('categories', get_categories_tree()); // fd30显示全部分类树
    ...
    //新增分类页商品相册 fd30
	$smarty->assign('cat_name',   $cat['cat_name']);//fd30 分类名称
		/********************************************************************************************/  
    //在分类页面调用当前分类以及其二级分类的信心，再在每个分类下调用5个商品  fd30>lib_goods.php 
    $cats = get_children_tree($cat_id);//获取当前分类信息以及子分类信息  
    $cat_detail = array();  
    foreach ($cats as $key => $val)  
    {  
        foreach($cats[$key]['children'] as $_k =>$_v){  
            $cats[$key]['children'][$_k]['goods_detail'] = get_cat_id_goods_list($_v['id'],12);  
...
function category_get_goods($children, $brand, $min, $max, $ext, $size, $page, $sort, $order)
{
    ...
    /* 获得商品列表 */
    $sql = 'SELECT g.goods_id, g.goods_name, g.goods_name_style, g.click_count, g.market_price, g.is_new, g.is_promote, g.is_best, g.is_hot, g.shop_price AS org_price, ' .
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
	    ...
	    $arr[$row['goods_id']]['pictures']             = get_goods_gallery($row['goods_id']);//fd30 商品相册
			   		//fd30修改(新品、热门标签)
		...
		$arr[$row['goods_id']]['buysum']          = ec_buysum($row['goods_id'])+get_cum_sales($row['goods_id']); //fd30 已销售	
	    $arr[$row['goods_id']]['evaluation']   = get_evaluation_sum($row['goods_id']);/*fd30评价数===调用get_evaluation_sum*/
    }
