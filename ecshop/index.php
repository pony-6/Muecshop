...
$categorys = $db->getAll("SELECT * FROM " . $ecs->table("category")." where is_show = 1 order by  sort_order asc");
foreach ($categorys as $k => $cate){
    $children = get_children($cate['cat_id']);
    $categorys[$k]['goods'] = get_category_recommend_goods('best', $children, 0, 0,  0, '',8);
}
//echo "<pre>";
//print_r($categorys);
