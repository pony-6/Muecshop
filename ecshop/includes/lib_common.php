...
/*记录积分*/
function insert_account_log($user_id,$pay_points,$order_id,$push_type='0')
{
    if(!$pay_points){
        return false;
    }
    ...
    $msg = "订单支出";
    $type = "99";
    $sql ="insert into ".$GLOBALS['ecs']->table('account_log')." (user_id,pay_points,user_money,change_time,change_desc,change_type) values ('".$user_id."','-".$pay_points."','".$order_points."','".time()."','".$msg."','".$type."')";
    $GLOBALS['db']->query($sql);
    /* 用户减积分 */
    $sql = "select pay_points from ecs_users where user_id = ".$user_id;
    $pay_points1 =  $GLOBALS['db']->getRow($sql);
    $sum = $pay_points1['pay_points']-$pay_points;
    $sql = "update ecs_users set pay_points = ".$sum." where user_id =".$user_id;
    $GLOBALS['db']->query($sql);
