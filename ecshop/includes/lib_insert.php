...
function insert_history()
{
    ...
    if (!empty($_COOKIE['ECS']['history']))
    {
        ...
        while ($row = $GLOBALS['db']->fetch_array($query))
        {
            ...
            $str.='					 <li> 
					 <a href="'.$goods['url'].'" target="_blank" title="'.$goods['goods_name'].'"> <img alt="'.$goods['goods_name'].'" src="'.$goods['goods_thumb'].'"><h3>'.$goods['goods_name'].'</h3>
...
function insert_ads($arr)
{
    ...
    {
        $sql  = 'SELECT a.ad_id,a.link_man,a.link_email,a.link_qt,a.position_id, a.media_type, a.ad_link, a.ad_code, a.ad_name, p.ad_width, ' .
    ...
    foreach ($res AS $row)
    {
        ...
        switch ($row['media_type'])
        {
		  ...
		  if($row['link_man']){
                ...
                border='0' /><span class=ad_name>$row[ad_name]</span><span class=link_man>$row[link_man]</span></a>";
					   
					   }
					
			   else{	 $ads[]= "<a class='a_$row[link_email]' cat='$row[link_qt]' href='$row[ad_link]'
                target='_blank'><img src='$src' width='" .$row['ad_width']. "' height='$row[ad_height]'
                border='0' /></a>";}
...
function api_insert_ads($ad_id,$cat_id)
{      
  $time = gmtime();	
  $sql  ="select ad_link,ad_name,link_man,link_email,ad_code,link_qt from ".$GLOBALS['ecs']->table('ad')." where enabled = 1 AND start_time <= '" . $time . "' AND end_time >= '" . $time . "' and position_id = '$ad_id'  and link_qt ='$cat_id'";
    $res = $GLOBALS['db']->GetAll($sql);
    $ads = array();
	foreach ($res AS $idx => $row) 
    {
		$ads[$idx]['id']=$row["ad_link"];
		$ads[$idx]['link_email']=$row["link_email"];
