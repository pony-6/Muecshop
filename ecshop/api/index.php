...
if (!$smarty->is_cached('home.dwt', $cache_id))
{
    assign_template();
    ...
    //$smarty->assign('sort_goods_arr',    index_get_topic());  	
...
/********************循环调用一级分类商品************************************************************************/  
$cats = get_categories_tree(0);//获取当前分类信息以及子分类信息
$cat_detail = array();  
foreach ($cats as $key => $val)  
{  
	$cats[$key]['goods_detail'] = get_cat_id_goods_listss($val['id'],12);  
	$cats[$key]['ad_info'] = api_insert_ads(206,$val['id']);  
	$cat_detail = $cats;//当前分类下的二级分类及其商品信息  
}  
...
{
$sql = "SELECT article_id,description,title,open_type,cat_id,file_url FROM " .$GLOBALS['ecs']->table('article'). " WHERE cat_id = ".$cat_aid." and is_open = 1 LIMIT " . $cat_num; 
$res = $GLOBALS['db']->getAll($sql); 
$arr = array(); 
foreach ($res AS $idx => $row) 
{ 
