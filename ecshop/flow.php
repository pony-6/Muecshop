...
elseif ($_REQUEST['step'] == 'consignee')
{
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'GET')
    {
        ...
        if (isset($_REQUEST['direct_shopping']))
        {
            ...
            exit('<script>top.location.href="flow.php?step=checkout"</script>');//fd30
        }
...
//    }
    ...
    if ($_CFG['anonymous_buy']==0 && $_SESSION['user_id'] == 0)
    {
        /* 用户没有登录且没有选定匿名购物，转向到登录页面 */
        ecs_header("Location: flow.php?step=login\n");
        exit;
    ...
    foreach ($cart_goods as $k =>$v){
        $sql ="select goods_thumb from ecs_goods where goods_id =".$v['goods_id'];
        $goods_thumb[$k] = $GLOBALS['db']->getrow($sql);
        $cart_goods[$k]['goods_thumb'] = $goods_thumb[$k]['goods_thumb'];
    }
...
elseif ($_REQUEST['step'] == 'update_cart1')
{
    if (isset($_POST['goods_number']) && is_array($_POST['goods_number']))
    {
        flow_update_cart($_POST['goods_number']);
    ...
    ecs_header("Location: flow.php"); //fd30修改
