      ...
      for (var j = 0; j < myTable.rows[i].cells.length; j++)
      {
        ...
        myTable.rows[i].cells[j].onmousedown = function()
        {
         ...
         document.getElementById("goods_name_color").value = this.bgColor;
		  $(".word_c,.fd30_cat ul li .cat_p dl dd a.word_c").css({"color":this.bgColor}); 
		  $(".bg_c,.Tr_cs .TABLE_S b.visited").css({"background-color":this.bgColor}); 
		  $(".border_c").css({"border-color":this.bgColor});  		   		  
