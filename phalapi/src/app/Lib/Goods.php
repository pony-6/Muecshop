...
class Goods
{
    ...
    public function getPinTuanGoodsList(){
        $data = $this->goods->getPinTuanGoods();
        return $data;
    }

    public function getHotGoodsList()
    {
        $data = $this->goods->getHotGoods();
        return $data;
    }
    ...
    public function getGoodsSkuList($goods_id)
    {
        // 返回商品数据配置
        ...
        if (count($goods_attr_array) > 1) { // 组合属性
                ...
                foreach ($goods_attr as $k => $v) {
                    ...
                    for ($i = 0; $i < count($goods_sku); $i++) {
                        ...
                        if ($goods_sku[$i]['product_id'] == $goods_attr[$k]['product_id']) {
                            ...
                            $goods_sku_array[$goods_sku[$i]['goodsCode']]['price'] = $this->price($goods_sku, $goods_attr[$k]['product_id']);
            ...
            foreach ($data as $k => $v) {
                if ($data[$k]['attr_type'] != 2) { // 去除可多选
                    ...
                    $pz[$data[$k]['goodsCode']]['price'] = $data[$k]['price'] + $data[$k]['attr_price'];
    ...
    public function price($goods_sku, $product_id)
    {
        // 计算价格
