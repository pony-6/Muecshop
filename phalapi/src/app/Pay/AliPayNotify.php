...
class AliPayNotify extends NotORM
{
    ...
    public function Notify($data)
    {
        ...
        $aop->rsaPrivateKey = $ali_config['rsaPrivateKey']; //请填写开发者私钥去头去尾去回车，一行字符串

        $aop->format = "json";
        $aop->charset = "UTF-8";
        $aop->signType = "RSA2";
        $aop->alipayrsaPublicKey = $ali_config['alipayrsaPublicKey']; //请填写支付宝公钥，一行字符串
        ...
        if ($param['out_trade_no'] && $param['total_amount']) {
            ...
            $sql = "update ecs_order_info set pay_time='" . time() . "', order_status = '1',pay_status = '2',pay_name = '支付宝'  WHERE order_sn = '" . $param['out_trade_no'] . "'";
