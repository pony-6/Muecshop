        ...
        if ($xmlObj['return_code'] == "SUCCESS" && $xmlObj['result_code'] == "SUCCESS") {
            ...
            if ($sign === $xmlSign) {
                ...
                if ($data['attach'] == 'deposit') {
                    ...
                    $sql = "update ecs_order_info set pay_time='" . time() . "', order_status = '1',pay_status = '2',pay_name='微信'  WHERE order_sn = '" . $data['out_trade_no'] . "'";
                    $this->getORM()->queryRow($sql);

                    $sql = "select user_id from  ecs_order_info  WHERE order_sn = '" . $data['out_trade_no'] . "'"; // 获得用户ID
                    $res = $this->getORM()->queryRow($sql);

                    $sql = "insert into ecs_account_log set user_id = '" . $res['user_id'] . "',user_money = '-" . $data['total_fee'] / 100 . "',frozen_money = '0',rank_points = '0',pay_points = '0',change_time = '" . time() . "',change_desc = '微信支付订单 " . $data['out_trade_no'] . "',change_type='99' "; // 记录该次消费
