    ...
    public function getRules()
    {
        return array(
            ...
            'checkLoginApi' => array(
                ...
                'token' => array('name' => 'token', 'require' => false, 'min' => 1, 'max' => 50, 'desc' => 'token'),
            ),
            ...
            "getWxCodeApi" => array(
                "order_id" => array("name"=>"order_id","require"=>false,"min"=>0,"desc"=>"订单号")
            ),
            "getOpenid" => array(
                "code" => array("name"=>"code","require"=>true,"min"=>1,"desc"=>"code"),
            )
...
//        }
        ...
        $user_data = $this->model->mobile_register($user_name, $password,$platform,$parent_id);
...
//        var_dump($user_data);exit;
    ...
    public function checkWechatRegApi()
    {
        ...
        $res = $this->model->checkWechatReg($openid,$platform);   // 需要判断一下平台

        if ($res['res'] === true) { // 已注册 直接登录
            $user_data = $this->model->wechatLogin($openid,$platform);
            $sess_id = $this->token_model->set_sess($user_data);
    ...
    public function bindingWechatApi()
    {
        ...
        if($user_check)
        {
            ...
            $res = $this->model->bindWechat($mobile,$vcode,$openid,"reg",$platform);   // 进行注册绑定操作
            if(!$res){
                return array('res'=>false,"msg"=>"验证码不正确");
            }
            $sess_id = $this->token_model->set_sess($res);
            return array('res' => true, 'user_id' => $user_data['user_id'], 'sess_id' => $sess_id);
        }
        else
        {
            // 已经注册过了  直接进行绑定操作

            $res = $this->model->bindWechat($mobile,$vcode,$openid,"bind",$platform);
            // 绑定完成之后就进行登录操作
            if(!$res){
                return array('res'=>false,"msg"=>"验证码不正确");
            }
    ...
    public function loginWechatApi()
    {
        ...
        $url = 'https://api.weixin.qq.com/sns/jscode2session?appid='.$wx['id'].'&secret='.$wx['secret'].'&js_code='.$code.'&grant_type=authorization_code';
        $wx_config = json_decode($this->getWechatMsg($url));   // 获取到微信的openid
        ...
        $res = $this->model->checkWechatReg($wx_config->openid,$platform);

        if ($res['res'] === true) { // 已注册 直接登录
            $user_data = $this->model->wechatLogin($wx_config->openid,$platform);
            $sess_id = $this->token_model->set_sess($user_data);
...
//        $cfg_baseurl = $_SERVER['SERVER_NAME'];
       $cfg_baseurl = 'http://www.mushiyuan.com.cn';

        $back_url = $cfg_baseurl.'/h5/apiPam/logincommon/main';


        $redirect_url = urlencode($back_url);
    ...
    public function getWXUserInfoApi(){
        ...
        $config = \PhalApi\DI()->config->get('app');
        $appid = $config['wxpay_appid'];  // 微信分配的平台账号
        $appsecret = $config['appsecret'];  // app secret
        ...
        $res = $this->model->checkWechatReg($openid,'H5');   // 判断这个用户是否存在
        ...
        if ($res['res'] === false){ //没有注册 需要将数据写到表中
            ...
            $status = $this->model->bindWXH5($nickname,$openid,$randpwd);
            if($status){
                $user_data = $this->model->wechatLogin($openid,'H5');
                $sess_id = $this->token_model->set_sess($user_data);
    ...
    //H5 微信回调获取openid之前所需的code
    public function getWxCodeApi(){
 ...
 //       $back_url = $cfg_baseurl.'/h5/apiPam/logincommon/main';
        $back_url = $cfg_baseurl.'/h5/apiCart/pay/main?orderId='.$order_id;
