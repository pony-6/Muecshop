...
//        die;
    ...
    public function get_goods_properties($goods_id)
    {
        ...
        $sql = "SELECT goods_id AS id,goods_name AS name,goods_thumb AS list_pic_url,shop_price AS retail_price FROM ecs_goods WHERE cat_id={$cat_id} ORDER BY sales_volume_count DESC LIMIT 6";
...
//            $where = $this->getORM()->db_create_in($spec, 'goods_attr_id');
            ...
            $sql = "SELECT SUM(attr_price) AS attr_price FROM ecs_goods_attr WHERE goods_attr_id".$this->getORM()->db_create_in($spec);
            $price = $this->getORM()->queryRows($sql, $params);
            $price = current($price);
    ...
    public function searchGoods($words,$order = 'asc',$page,$num){
        $page_size = pagesize();
        ...
        $sql = "SELECT goods_id AS id,goods_name AS name,shop_price AS retail_price,goods_thumb AS list_pic_url FROM ecs_goods WHERE goods_name LIKE '%{$words}%' ORDER BY shop_price {$order} LIMIT {$offset},{$page_size}";
        $res = $this->getORM()->queryRows($sql);
...
//        $data = $this->getORM()->order("add_time DESC")->limit(5)->fetchAll();
    ...
    public function getNewGoodsList($order,$page,$isHot,$spike,$pintuan,$isNew,$type){
        // 新品首发
        $page_size = pagesize();
        $offset = ($page - 1) * $page_size;
        if($type =='price'){
        ...
        if($order != '' && $spike == "" && $pintuan == "" && $isHot == ''){//如果为空默认显示首页，不为空显示价格
            $sql = "SELECT *  FROM ( SELECT goods_name AS name, goods_id AS id, shop_price AS retail_price, goods_thumb AS list_pic_url 
            ...
            $sql = "select goods_id as id,goods_name as name,goods_thumb as list_pic_url,pt_price as retail_price from ecs_goods where
            ...
            $sql = "select goods_id as id,goods_name as name,goods_thumb as list_pic_url,pt_price as retail_price,sales_volume_count as num from ecs_goods where 
                   is_on_sale = 1 and is_delete = 0 and goods_number>0 and is_pintuan = '1' order by num {$isHot} limit ".$offset.",".$page_size;
        }else{
            $sql = "SELECT goods_name AS name, goods_id AS id, shop_price AS retail_price, goods_thumb AS list_pic_url FROM ecs_goods where
        ...
        if ($isHot != '1' && $order == "" && $spike == '' && $pintuan == ''){
            $sql = "SELECT *  FROM ( SELECT goods_name AS name, goods_id AS id, shop_price AS retail_price, goods_thumb AS list_pic_url,sales_volume_count AS num 
        ...
        if ($order == "" && $isHot == "" && $spike == "" && $pintuan != ""){
            $sql = "select goods_id as id,goods_name as name,goods_thumb as list_pic_url,pt_price as retail_price,spike_count from ecs_goods where
...
//        $data = $this->getORM()->order("click_count DESC")->limit(5)->fetchAll();
        foreach($data as $k => $v){
            $data[$k]['goods_thumb'] = goods_img_url($data[$k]['goods_thumb']);
        }
        return $data;
