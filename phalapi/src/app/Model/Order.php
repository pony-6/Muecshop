...
//        die;
        if($delivery_id != ""){
            ...
            $update = "update ecs_order_info set address = '$location' where order_id = '$order_id'";
...
//            var_dump($product_id);die;
    ...
    function cart_weight_price($user_id)
    {
...
//        $str = "select parent_id from ecs_users where user_id = $user_id"; //判断这个账户是否是分享注册的账户
//        $sth = $this->getORM()->queryRow($str);
//        if($sth){
...
//            $separate_by = $tmp['config']['separate_by'];
//            $cash = $level_money_all * $order_data['total_fee'] * 0.01;
//            $sql = "select order_id from ecs_order_info where order_sn='$order_id'";
//            $res =  $this->getORM()->queryRow($sql);
//            $sql = "select give_integral,shop_price from ecs_goods,ecs_order_goods where ecs_goods.goods_id = ecs_order_goods.goods_id and order_id =".$res['order_id'];
//            $points = 0;
//            $ss =  $this->getORM()->queryRows($sql);
//
//            foreach ($ss as $row){
//                if($row['give_integral'] == '-1')
//                {
//                    $points += $level_point_all * $row['shop_price'] * 0.01;
//                }
//                else{
//                    $points += $level_point_all * $row['give_integral'] * 0.01;
//                }
//            }
//
//            $sql = "select user_name from ecs_users where user_id = ".$sth['parent_id'];
//            $ss = $this->getORM()->queryRow($sql);
////            $this->accountlog->write_affiliate_log($res['order_id'],(int)$sth['parent_id'],$ss['user_name'],$cash,$points,$separate_by);
//
//
//        }
...
//        $this->accountlog->log_account_change((int)$sth['parent_id'],$cash,0,0,$points,sprintf('返利',$order_id));

//        var_dump((int)$sth['parent_id']);exit;

        $this->accountlog->log_account_change($user_id, $order_data['total_fee'] * (-1), 0, 0, 0, sprintf('订单支付', $order_id));
...
//            var_dump($goods_id);die;
    ...
    /* 截单 */
    function  getOrder_end($product_id,$user_id){
