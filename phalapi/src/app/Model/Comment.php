...
class Comment extends NotORM
{
    ...
    public function addGoodsComment($user_id, $goods_id, $order_id, $comment, $comment_rank, $anonymous)
    {
        ...
        if ($anonymous == true) {
            // 匿名
            $sql = "INSERT INTO ecs_comment SET comment_type='0',id_value={$goods_id},email='" . $data[0]['email'] . "',content='" . $comment . "',comment_rank={$comment_rank},add_time='" . time() . "',ip_address='" . $_SERVER["REMOTE_ADDR"] . "',status='1',user_id='0',order_id={$order_id},app_user_id={$user_id} ";
            $this->getORM()->queryRows($sql);
            return true;
        }
        $sql = "INSERT INTO ecs_comment SET comment_type='0',id_value={$goods_id},email='" . $data[0]['email'] . "',user_name='" . $data[0]['user_name'] . "',content='" . $comment . "',comment_rank={$comment_rank},add_time='" . time() . "',ip_address='" . $_SERVER["REMOTE_ADDR"] . "',status='1',user_id={$user_id},order_id={$order_id},app_user_id={$user_id} ";
        $this->getORM()->queryRows($sql);
        return true;
    }
